...
La gestion des utilisateurs se fait ensuite grâce à un "token" unique délivré par le service tiers.
...
C'est le cas par exemple de Deezer ou de StackOverflow qui proposent de s'identifier avec un compte Google, Facebook, Twitter, Windows Live ou autre.
...
Nous avons alors décider d'utiliser OAuth 2.0 car nous ne souhaitions pas mettre en place et administrer une base de donnée pour les utilisateurs.
D'autre part nous ne voulions pas avoir à gérer la sécurité des données stockés.
...
\begin{figure}[!h]
	\center
	\includegraphics[width=0.6\textwidth]{img/OAuth.png}
	\caption{OAuth : fonctionnement}
	Source : \href{http://www.siteduzero.com/informatique/tutoriels/creez-des-applications-pour-windows-phone/qu-est-ce-qu-oauth}{http://www.siteduzero.com}
	\label{OAuth}
\end{figure}

Comme le montre le schéma \ref{OAuth}, l'authentification se compose de plusieurs étapes.
...
son token avec les serveurs de Google dans le but d'obtenir un token d'accès. Si tout se déroule correctement elle obtient le token d'accès.
